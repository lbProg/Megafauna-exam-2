...
The best model for time-dependent p was **model 1**, so we used this model as the basis for selecting the best model for p that also included sex.
...
mcmc_p.sex_phi.timeCAT$summary
```
...
mcmc_p.sex_phi.timeCONT$summary
```
...
|   2   |    cat    |     RE      |      cat      |      cat       |    553    |
|   3   |    cat    |  constant   |      cat      |      cat       |    557    |
|   4   | constant  |  constant   |      cat      |      cat       |    537    |
|   5   |    cat    |  constant   |      cat      |       RE       |    533    |
|   6   |    cat    |  constant   |      cat      |   continuous   | **529\*** |
|   7   |    cat    |  constant   |      cat      |    constant    |    535    |
|   8   |    cat    |  constant   |   constant    |       RE       |    531    |

cat = categorical ; RE = random effect

**\***Best model

**Model 8** and **model 6** were considered equivalent because their $\Delta$WAIC is ≤ 2.

However, **model 6** is more parsimonious (less parameters). Therefore, we selected **model 6** for subsequent analyses.
...
pool_n <- function(data, sex, n_female, n_male) {
  ...
  groups[sex == 2 &  (data %in% topD_f)] <- max(levels)
  groups[sex == 2 & !(data %in% topD_f)] <- 1
  groups[sex == 1 &  (data %in% topD_m)] <- max(levels)
  groups[sex == 1 & !(data %in% topD_m)] <- 1

  match(groups, sort(unique(groups)))
}

```

```{r}
data.frame(x = oxyd, SEX = raw_data$sex, y = pool_n(oxyd, raw_data$sex, n_female = 1, n_male = 2))
```
...
mcmc_p.sex_phi.timeCAT.oxyCONT$summary
```
...
hmm.p.sex_phi.timeCAT.oxyCAT <- nimbleCode({
  ...
  for(i in 1:N){
    for(t in 1:(T-1)){
      gamma[1,1,i,t] <- phi[oxyd[i],t]      # Pr(alive t -> alive t+1)
      gamma[1,2,i,t] <- 1 - phi[oxyd[i],t]  # Pr(alive t -> dead t+1)
...
mcmc_p.sex_phi.timeCAT.oxyCAT$summary

```
We can observe that survival tends to decrease from the 9th or 10th decile. Therefore, it might make sense to group the first 9 or 8 deciles together and keep the last one separate. 

### $log(OXY)$ as a pooled categorical variable (9 first deciles)
...
mcmc_p.sex_phi.timeCAT.oxyCATD9$summary
```

### $log(OXY)$ as a pooled categorical variable (8 first deciles)
...
mcmc_p.sex_phi.timeCAT.oxyCATD8$summary
```
### $log(OXY)$ as a categorical variable in interaction with sex
...
hmm.p.sex_phi.timeCAT.oxyCAT_intsex <- nimbleCode({
  ...
  for(i in 1:N){
    for(t in 1:(T-1)){
      gamma[1,1,i,t] <- phi[oxyd[i], SEX[i], t]      # Pr(alive t -> alive t+1)
      gamma[1,2,i,t] <- 1 - phi[oxyd[i], SEX[i], t]  # Pr(alive t -> dead t+1)
      gamma[2,1,i,t] <- 0           # Pr(dead t -> alive t+1)
      gamma[2,2,i,t] <- 1           # Pr(dead t -> dead t+1)
    }
  }
...
```{r}
params <- c("p", "phi","alpha_phi0", "beta_oxy", "beta_time", "beta_int", "beta_sex")
```
...
mcmc_p.sex_phi.timeCAT.oxyCAT_intsex$summary
```

For sex 1, the first nine deciles have similar survival, while survival decreases for the tenth decile. For sex 2, the first eight deciles have similar survival, whereas survival decreases for the last two deciles. Sex 1 is supposed to correspond to males and sex 2 to females. However, in the article, it’s the opposite: the first nine deciles correspond to females and the first eight deciles to males. Nevertheless, we chose to remain consistent with our own data in order to maintain a coherent approach when selecting the best model. Therefore, we subsequently tested grouping the first nine deciles for males and the first eight deciles for females.

### $log(OXY)$ as a categorical variable in interaction with sex : 9 first deciles grouped for males and 8 first deciles grouped for females. 
...
mcmc_p.sex_phi.timeCAT.oxyCAT_D8FD9M_intsex$summary
```


### $log(OXY)$ as a categorical variable : 9 first deciles grouped for males and 8 first deciles grouped for females. 

In this model, there is no interaction with sex. This means that the high levels of oxy, even if they are not the same for females and males, act the same on survival.
