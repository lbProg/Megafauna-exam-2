...
mcmc_p.sex_phi.timeCAT.oxyCONT <- nimbleMCMC(
  code = hmm.p.sex_phi.timeRE.oxyCONT,
  constants = my.constants,
  data = my.data,
  inits = initial.values,
  monitors = params,
...
mcmc_p.sex_phi.timeCAT.oxyCAT2D <- nimbleMCMC(
  code = hmm.p.sex_phi.timeRE.oxyCAT2D,
  constants = constants,
  data = my.data,
  inits = initial.values,
  monitors = params,
...
```{r}
makeinits <- function() {
  B <- matrix(rnorm(20, 0, 1), nrow=K, ncol=G)
  B[1,1] <- 0
...
```{r}
mcmc_p.sex_phi.timeRE.oxyCAT_2DF3DM  <- nimbleMCMC(
  code = hmm.p.sex_phi.timeRE.oxyCAT_2DF3DM,
  constants = constants,
  data = my.data,
  inits = initial.values,
  monitors = params,
