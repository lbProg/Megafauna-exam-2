...
makeinits <- function() {
  list(
    mup = rnorm(1, 0, 1),
    sdeps_p = runif(1, 0, 3),
    beta_p_sex = c(0, rnorm(K - 1, 0, 1.5)),
    alpha_phi0 = rnorm(1, 0, sd = 1.5),
    beta_phi_time = c(0, rnorm(T - 2, 0, sd = 1.5)),
    beta_phi_sex = c(0, rnorm(K - 1, 0, sd = 1.5)),
    z = t(apply(y, 1, zinits))
...
Survival may be the same between sexes if their behavior is similar to their reproductive investment.
...
initial.values <- rep(get_init_vals(), 4)
```

```{r}
params = c("p", "muphi","sdeps_phi", "beta")
```

```{r}
mcmc_oxyCONT <- nimbleMCMC(
  code = model_oxyCONT,
  constants = my.constants,
  data = my.data,
  inits = initial.values,
...
initial.values <- rep(get_init_vals(oxyd), 4)
```
...
```{r}
initial.values <- rep(get_init_vals(oxyd_2D), 4)
```

```{r}
mcmc_oxyCAT2D <- nimbleMCMC(
  code = model_oxyCAT,
  constants = constants,
  data = my.data,
  inits = initial.values,
...
```{r}
initial.values <- rep(get_init_vals(oxyd_3D), 4)
```

```{r}
mcmc_oxycat3D <- nimbleMCMC(
  code = model_oxy_cat,
  constants = constants,
  data = my.data,
  inits = initial.values,
