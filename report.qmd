...
```{r}
params <- c("p", 
            "muphi","beta_sex_phi","sdeps_phi", "eps_phi")
...
```{r}
params <- c("p",
            "alpha_phi0","beta_time","beta_phi_sexe","p","phi")
...
get_init_vals <- function(data) {
  list(
    beta = rnorm(length(unique(data)), 0, 1),
    p = runif(2, 0, 1),
    z = t(apply(y, 1, zinits))
  )
}
...
```{r}
initial_values <- rep(get_init_vals(oxyd_mf), 4)
```
